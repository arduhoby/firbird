...
class _LiveAudioRecordingScreenState
  ...
  Future<void> _mergeWavSegments(List<String> segmentPaths, String destination) async {
    ...
    final output = BytesBuilder(copy: false)..add(header)..add(audio);
    await File(destination).writeAsBytes(output.takeBytes(), flush: true);
    for (final segmentPath in segmentPaths) {
      try { await File(segmentPath).delete(); } catch (_) {}
    }
  }
  ...
  Future<bool?> _reviewDetection(LiveDetectionEntry entry) async {
    ...
    final bool? correct = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Doğru değil')),
          FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Doğru')),
        ],
      ),
    );
    ...
    if (correct) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${entry.prediction.turkishName} doğru olarak işaretlendi.')),
      );
      return false;
    }
    setState(() {
